...
const skillGroups = [
  ...
  },
  {
    category: "AI & Tools",
    skills: [
      { name: "OpenAI API Integration", level: 78 },
      { name: "Git & GitHub", level: 87 },
      { name: "VS Code", level: 95 },
      { name: "Postman", level: 82 },
      { name: "Vercel / Netlify Deploy", level: 88 },
    ],
  },
];
...
function SkillBar({ name, level, visible, delay }) {
  ...
  useEffect(() => {
    if (visible) { const t = setTimeout(() => setAnimated(true), delay); return () => clearTimeout(t); }
  }, [visible, delay]);
  ...
  return (
    <div style={{ marginBottom: "16px" }}>
      ...
      <div style={{ height:"3px", background:"rgba(255,255,255,0.06)", borderRadius:"3px", overflow:"hidden" }}>
        <div style={{
          height:"100%", width: animated ? `${level}%` : "0%",
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"50px" }}>
          {skillGroups.map((group, gi) => (
            <div key={group.category} style={{
              opacity: visible ? 1 : 0,
              transform: visible
                ? `translate(${mouse.x*(gi%2===0?-4:4)}px, ${mouse.y*-3}px)`
                : "translateY(30px)",
              ...
              <div style={{
                ...
                <span style={{ color:"#4ade80", fontSize:"10px", background:"rgba(74,222,128,0.08)", padding:"2px 10px", borderRadius:"20px" }}>
