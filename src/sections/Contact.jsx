...
export default function Contact() {
  ...
  const input={width:"100%",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:"8px",padding:"12px 14px",color:"#fff",fontSize:"13px",outline:"none",boxSizing:"border-box",transition:"border-color 0.2s",fontFamily:"inherit"};
  ...
  return (
    <section id="contact" ref={ref} style={{
      ...
      <div style={{maxWidth:"1100px",margin:"0 auto",width:"100%"}}>
        <div style={{marginBottom:"44px",opacity:visible?1:0,transition:"opacity 0.7s ease",transform:isMobile?"none":`translate(${mouse.x*-6}px,${mouse.y*-4}px)`}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:isMobile?"40px":"60px",alignItems:"start"}}>
          {/* Form */}
          <div style={{opacity:visible?1:0,transform:visible?isMobile?"none":`translate(${mouse.x*-7}px,${mouse.y*-4}px)`:"translateX(-30px)",transition:"opacity 0.7s ease 0.2s, transform 0.12s ease-out"}}>
            {sent?(
              ...
              <form onSubmit={(e)=>{e.preventDefault();setSent(true);}} style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                <input required placeholder="Your Name" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} style={input} onFocus={e=>e.target.style.borderColor="rgba(74,222,128,0.5)"} onBlur={e=>e.target.style.borderColor="rgba(255,255,255,0.1)"} />
                <input required type="email" placeholder="Your Email" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} style={input} onFocus={e=>e.target.style.borderColor="rgba(74,222,128,0.5)"} onBlur={e=>e.target.style.borderColor="rgba(255,255,255,0.1)"} />
                <textarea required placeholder="Your Message" rows={4} value={form.message} onChange={e=>setForm(f=>({...f,message:e.target.value}))} style={{...input,resize:"vertical"}} onFocus={e=>e.target.style.borderColor="rgba(74,222,128,0.5)"} onBlur={e=>e.target.style.borderColor="rgba(255,255,255,0.1)"} />
                <button type="submit" style={{background:"#4ade80",color:"#000",border:"none",padding:"13px 28px",borderRadius:"8px",fontWeight:700,fontSize:"13px",cursor:"pointer",transition:"all 0.2s",marginTop:"4px"}}
          ...
          {/* Socials */}
          <div style={{opacity:visible?1:0,transform:visible?isMobile?"none":`translate(${mouse.x*7}px,${mouse.y*4}px)`:"translateX(30px)",transition:"opacity 0.7s ease 0.3s, transform 0.12s ease-out"}}>
            <div style={{color:"rgba(255,255,255,0.3)",fontSize:"10px",letterSpacing:"0.15em",fontWeight:600,marginBottom:"16px"}}>FIND ME ON</div>
            {[{label:"GitHub",href:"https://github.com/Arushi026",sub:"github.com/Arushi026"},{label:"LinkedIn",href:"https://www.linkedin.com/in/arushi-dhigurvar-08556a236",sub:"arushi-dhigurvar"},{label:"Leetcode",href:"https://leetcode.com/u/Arush_i26/",sub:"Arush_i26"},{label:"Email",href:"mailto:arushisingh02602@gmail.com",sub:"arushisingh02602@gmail.com"}].map(({label,href,sub})=>(
            ...
            <div style={{background:"rgba(74,222,128,0.05)",border:"1px solid rgba(74,222,128,0.2)",borderRadius:"12px",padding:"20px",marginTop:"24px"}}>
              ...
              <div style={{color:"rgba(255,255,255,0.45)",fontSize:"12px",lineHeight:1.6}}>Open to full-time roles and internships. Response within 24 hours.</div>
            </div>
          </div>
        </div>
      </div>
